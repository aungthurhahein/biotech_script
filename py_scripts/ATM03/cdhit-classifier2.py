...
for x in cluster_list:
    ...
    # multi
    if len(x_split) > 2:
        ref_id = re.search(r'>PM\w+', x)  # ATM
        org_id = re.search(r'>PV_ATM\w+', x)  # ATM03
...
# codesnippets.write_file(Set1_Multi, "{0}_shrimp_S2".format(clstrfile))
# codesnippets.write_file(Set1_Singleton, "{0}_shrimp_S1".format(clstrfile))
codesnippets.write_file(Set2_Multi, "{0}_NonShrimp_S2".format(clstrfile))
codesnippets.write_file(Set2_Singleton, "{0}_NonShrimp_S1".format(clstrfile))
