...
idmapfile = sys.argv[10]  # id file
set_name = sys.argv[11]  # setname
...
for idmap in open_id:
    id_split = idmap.split()
    mira_tri_id.append(id_split[0].strip())
    mira_id.append(id_split[1].strip())
...
for x in clst_list:
    ...
    for C01 in C_List:
        tmp_string2 = ""
        tmp_string2 = set_name + '\t' + clustername + '\t' + 'C01_MIRA' + '\t'
        if C01.strip().strip('>') in mira_id:
            triid = mira_tri_id[mira_id.index(C01.strip().strip('>'))]
            tmp_string2 += C01 + '|' + triid + '\t'
            if triid.strip().strip('>') in de_id:
                for derecord in de_record[de_id.index(triid.strip().strip('>'))]:
                    tmp_string2 += derecord + '\t'
        print tmp_string2
    # C Unmap
    tmp_string3 = set_name + '\t' + clustername + '\t' + 'C01_MIRA_Unmap' + '\t' + str(len(CUnmap_list)) + '\t'
    Control_count = 0
    Mature_count = 0
    Survival_count = 0
