...
# get common id
cur.execute('select DISTINCT %s.ID from  %s left join %s on %s.transcript_id = %s.ID where %s.ID is not null' % (datatable, datatable, updatetable,updatetable,datatable,updatetable ))
...
def get_data(table,id):
    ...
    db2 = MySQLdb.connect(host="127.0.0.1",
                     user="root",
                      ...
                      unix_socket="/opt/lampp/var/mysql/mysql.sock")
    cur2= db2.cursor()
    cur2.execute('select gene_id, Transcript_id, sprot_Top_BLASTX_hit, TrEMBL_Top_BLASTX_hit, RNAMMER, prot_id, prot_coords, sprot_Top_BLASTP_hit, TrEMBL_Top_BLASTP_hit, Pfam, SignalP, TmHMM, eggnog, gene_ontology_blast, gene_ontology_pfam, transcript, peptide from %s where transcript_id = %s' % ( table,id ) )
...
def get_status(id):
    ...
    db3 = MySQLdb.connect(host="127.0.0.1",
                     user="root",
                      ...
                      unix_socket="/opt/lampp/var/mysql/mysql.sock")
    cur3= db3.cursor()
    cur3.execute('select blastx_source,blastp_source from %s where id= %s'%(datatable,id))
