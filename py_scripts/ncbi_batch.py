...
# TODO:: xml parsing and getting data
...
def parse_command_line():
    ...
    parser.add_argument('-reldate', '--reldate', help= "an integer n, the search returns by datetype within the last n days")
...
def main(db, term, datetype, reldate, mindate, maxdate, rettype):
    ...
    if query_key != "" and web_env != "" and record_count != "":
        ...
        esum_output = esum_response.readlines()
        write_file(str(esum_output), re.sub(" ", "_", term.strip())+"_summary.xml")
        ...
        if int(record_count) <= 10000:
            ...
            times = int(int(record_count)/10000)+1
            restart = 0

            for i in range(0, times):
                restart = int(i) * 10000
                print "downloading from" + str(restart) + " sequence of " + str(record_count) + str(i + 1) + " time"
                values = {
                    'db': db,
                    'query_key': query_key,
                    'WebEnv': web_env,
                ...
                # post the efetch URL
                time.sleep(60)   # delay for 1 min
                data = urllib.urlencode(values)
                req = urllib2.Request(url, data)
                response = urllib2.urlopen(req)
                output = response.readlines()
                write_file(output, re.sub(" ", "_", term.strip())+"_records.gb_"+str(i+1))  # write to gbank file
            exit(1)
