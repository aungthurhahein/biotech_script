...
# get common id
cur.execute('select DISTINCT %s.gene_id from  %s left join %s on %s.gene_id = %s.gene_id where %s.ID is not null' % (datatable, datatable, updatetable,updatetable,datatable,updatetable ))
...
def get_data(table,id):
    ...
    db2 = MySQLdb.connect(host="127.0.0.1",
                     user="root",
                      ...
                      unix_socket="/opt/lampp/var/mysql/mysql.sock")
    cur2= db2.cursor()
    cur2.execute('select gene_id, Transcript_id, sprot_Top_BLASTX_hit, TrEMBL_Top_BLASTX_hit, RNAMMER, prot_id, prot_coords, sprot_Top_BLASTP_hit, TrEMBL_Top_BLASTP_hit, Pfam, SignalP, TmHMM, eggnog, gene_ontology_blast, gene_ontology_pfam, transcript, peptide from %s where gene_id = %s' % ( table,id ) )
...
def get_status(id):
    ...
    db3 = MySQLdb.connect(host="127.0.0.1",
                     user="root",
                      ...
                      unix_socket="/opt/lampp/var/mysql/mysql.sock")
    cur3= db3.cursor()
    cur3.execute('select blastx_source,blastp_source from %s where gene_id= %s'%(datatable,id))
...
if len(common) <= 0:
    ...
    for id in common:
        ...
        for x in range(0,len(data_exist)):
            ...
            cur4 = db4.cursor()
            cur4.execute('update %s Set gene_id = %s,Transcript_id= %s,sprot_Top_BLASTX_hit= %s,TrEMBL_Top_BLASTX_hit= %s,RNAMMER= %s,prot_id= %s,prot_coords= %s,sprot_Top_BLASTP_hit= %s,TrEMBL_Top_BLASTP_hit= %s,Pfam= %s,SignalP= %s,TmHMM= %s,eggnog= %s,gene_ontology_blast= %s,gene_ontology_pfam= %s,transcript= %s,peptide= %s,Blastx_source=%s,Blastp_source=%s where gene_id= %s'
                         % (datatable,final_update[0],final_update[1],final_update[2],final_update[3],final_update[4],final_update[5],final_update[6],final_update[7],
            final_update[8],final_update[9],final_update[10],final_update[11],final_update[12],final_update[13],final_update[14],final_update[15],final_update[16],blastx_source,blastp_source,idformat))
            db4.commit()
            cur4.close
            db4.close
