...
# get common id
cur.execute('select DISTINCT %s.gene_id from  %s left join %s on %s.gene_id = %s.gene_id where %s.ID is not null' % (
...
def get_data(table, id):
    ...
    cur2 = db2.cursor()
    cur2.execute(
        'select gene_id, Transcript_id, sprot_Top_BLASTX_hit, TrEMBL_Top_BLASTX_hit, RNAMMER, prot_id, prot_coords, sprot_Top_BLASTP_hit, TrEMBL_Top_BLASTP_hit, Pfam, SignalP, TmHMM, eggnog, gene_ontology_blast, gene_ontology_pfam, transcript, peptide from %s where gene_id = %s' % (
        table, id ))
