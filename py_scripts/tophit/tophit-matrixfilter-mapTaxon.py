...
with open(blastn,'r') as f1:
    for line in f1:
        ...
        if line_split[0].split('-')[0] == "#D":
            n_cat.append(line_split[0])
            n_id.append(line_split[3])
            n_species.append(line_split[1])
            n_gi.append(line_split[7])
            n_taxon.append(line_split[9])
            n_div.append(line_split[2])
...
for x,tid in enumerate(idfile):
    ...
    if NX[x] == "N)":
        ...
        for nind in n_indexes:
            if n_cat[nind].split('-')[1] == "NCal":
                tmp_group = "6A"
            elif n_cat[nind].split('-')[1] == "NC2":
                tmp_group = "6C"
            elif n_cat[nind].split('-')[1] == "NC1":
                tmp_group = "6B"
            if tmp_group == group:
