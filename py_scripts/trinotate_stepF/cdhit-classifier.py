...
for x in cluster_list:
    ...
    # multi
    if len(x_split) > 2:
        ref_id = re.search(r'>c\w+', x)  # trinityID
        org_id = re.search(r'>PM_\w+', x)  # FKnown
    ...
    # singleton
        ...
        ref_id = re.search(r'>c\w+', x_split[1])  # trinityID
        org_id = re.search(r'>PM_\w+', x_split[1])  # Fknown
        if ref_id:
            Set1_Singleton.append(x)
        elif org_id:
            Set2_Singleton.append(x)
codesnippets.write_file(both_side, "{0}_G2".format(clstrfile))
codesnippets.write_file(Set1_Multi, "{0}_trinityID_S2".format(clstrfile))
codesnippets.write_file(Set1_Singleton, "{0}_trinityID_S1".format(clstrfile))
codesnippets.write_file(Set2_Multi, "{0}_FKnown_S2".format(clstrfile))
codesnippets.write_file(Set2_Singleton, "{0}_FKnown_S1".format(clstrfile))
